...
const temples = [
  ...
  { templeName: "Sydney Australia", location: "Sydney, Australia", dedicated: "1984, September, 20", area: 39600, imageUrl: "https://www.churchofjesuschrist.org/imgs/8dd109cda45dda79ebe30b0461d5d0afba41f653/full/500%2C/0/default" },
  { templeName: "Johannesburg South Africa", location: "Johannesburg, South Africa", dedicated: "1985, August, 24", area: 19000, imageUrl: "https://www.churchofjesuschrist.org/imgs/b378c080e5880db5bf2bcf6d828b2f3fd59820de/full/500%2C/0/default" }
];
...
function displayTemples(templesArray) {
  ...
  templesArray.forEach(({ templeName, location, dedicated, area, imageUrl }) => {
    ...
    figcaption.innerHTML = `<strong>${templeName}</strong><br>Location: ${location}<br>Dedicated: ${dedicated}<br>Area: ${area} sq. ft.`;
...
function filterTemples(filter) {
  ...
  if (filter === "old") {
    filteredTemples = temples.filter(({ dedicated }) => parseInt(dedicated) < 1900);
  } else if (filter === "new") {
    filteredTemples = temples.filter(({ dedicated }) => parseInt(dedicated) > 2000);
...
// Navigation filter event listener
document.querySelector("nav").addEventListener("click", (event) => {
  const link = event.target.closest("a");
  if (!link) return;
  event.preventDefault();
  filterTemples(link.textContent.toLowerCase().trim());
});
